...
class DOM {
  ...
  ifElement = (fn) => {
    const element = this.toDOM();
    // element ? fn() : null; //TODO
  }
  ...
  doWhat = (operation, className) => {
    const all = Array.from(document.querySelectorAll(this.element));
  ...
  scrollTop = (behavior = 'smooth') => {
    this.ifElement(() => this.toDOM().scrollIntoView({ behavior }));
    return this;
  }
